...
def get_loftr_matches(matcher, img1, img2, device, match_max_dim=1200):
...
def mask_for_row(row_name, h, w):
    ...
    if row_name == "Horizon":
        ...
        # Cut off the bottom 20% of the Top row (was 30%)
        cut_h = int(h * 0.80)
        mask[cut_h:, :] = 0
    elif row_name == "Bottom":
        # Cut off the top 20% of the Bottom row (was 30%)
        cut_h = int(h * 0.20)
        mask[:cut_h, :] = 0
    return mask
